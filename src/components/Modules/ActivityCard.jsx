...
const ActivityCard = ({ activity }) => {
  ...
  // ✅ NEW: use activity.activityType to determine route
  const isDigital =
    activity?.activityType === "Digital" ||
    activity?.activityType === "Gamified";

  const href = isDigital
    ? `/digital/${activity?.id}`
    : `/interactive/${activity?.id}`;

  // ✅ NEW: use activity.coverImage — fallback to random color if "N.A" or missing
  const hasCoverImage =
    activity?.coverImage &&
    activity?.coverImage !== "N.A" &&
    activity?.coverImage !== "";

  const randomColor = `hsl(${Math.floor(Math.random() * 360)}, 70%, 80%)`;

  return (
    <Link href={href} className="min-w-[276px] max-w-[276px]">
      <div className="relative">
        ...
        <div className="absolute right-2 top-2 z-10">
          <span
            style={{
              background: isDigital ? "#548EFB" : "#FF8B13",
              color: "#fff",
      ...
      <div className="flex flex-col gap-2 rounded-[10px] bg-white p-4">
        ...
        <div className="body3_b flex justify-between text-primary">
          <h5>#{activity?.orderIndex || 1}</h5>
          <h5 style={{ color: "#FF8B13", fontSize: "12px", fontWeight: 700 }}>
            {activity?.activityType}
          </h5>
        </div>

        {/* ✅ NEW: Title from activity.title */}
        <h5 className="h5_b text-black">
          {activity?.title || "Untitled Activity"}
        </h5>

        {/* Age group + players row */}
        <div className="body_4 flex items-center gap-2 font-semibold text-grey_1">
          ...
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              ...
              stroke="#FF8B13" strokeLinecap="round" strokeLinejoin="round"
            />
            <path
